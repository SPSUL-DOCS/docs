...
Port na routeru např. _GigabitEthernet0/0/0_ se nastavují pomocí
...
```cisco title="Nastavení inside nat"
Router(config)#int g0/0/0
Router(config-if)#ip nat inside
```

```cisco title="Nastavení outside nat"
...
- First Hop Redundancy Protocols
  ...
  `standby <id> priority <priorita>` nastavení priority (0-255) defaultni = 100
- Vyšší priorita vyhrává
  `standby <id> preempt` router se bude snažit dát jako hlavní když se znovu zapne a bude mít vetší prioritu
